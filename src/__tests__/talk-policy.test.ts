...
describe('talk policy availability reasons', () => {
  ...
  test('resolveExecutionMode always returns full_control regardless of stored value', () => {
    // Even with executionMode: 'openclaw' in stored talk metadata,
    // resolveExecutionMode now always returns 'full_control'.
    const states = evaluateToolAvailability(
    ...
    // With full_control mode and toolMode: 'off', tools are blocked by tool_mode not execution_mode
    expect(states[0]?.enabled).toBe(false);
    expect(states[0]?.reasonCode).toBe('blocked_tool_mode');
  });

  test('marks tools blocked_tool_mode when approval is off', () => {
    const states = evaluateToolAvailability(
      [{ name: 'google_docs_create', description: 'docs', builtin: true }],
      {
        executionMode: 'full_control',
  ...
  test('blocks managed tools when proxy passthrough is disabled', () => {
    const states = evaluateToolAvailability(
  ...
  test('allows managed tools when proxy passthrough is enabled', () => {
    const states = evaluateToolAvailability(
      [{ name: 'google_docs_list_tabs', description: 'docs tabs', builtin: true }],
      {
        executionMode: 'full_control',
  ...
  test('blocks browser tools', () => {
    const states = evaluateToolAvailability(
      [{ name: 'browser_navigate', description: 'browser', builtin: true }],
      {
        executionMode: 'full_control',
        filesystemAccess: 'full_host_access',
        networkAccess: 'full_outbound',
    ...
    expect(states[0]?.enabled).toBe(false);
    expect(states[0]?.reasonCode).toBe('blocked_execution_mode');
    expect(states[0]?.reason).toBe('Browser control is not available.');
  });
});
